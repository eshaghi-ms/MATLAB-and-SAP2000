...
disp('clean-up the workspace & command window')
close all;
clear;
clc;
...
ModelDirectory = 'C:\Code\SapData\Simple\ACOCSiAPIexample';

if ~exist(ModelDirectory, 'dir')
    mkdir(ModelDirectory)
end

ModelName = 'API_Simple_AC0.sdb';
...
for i=1:NumberOfStory+1
    ...
    if Ret ~= 0
        disp(['! Error at AISC360_10.SetOverwrite NumberOfStory = ' num2str(i)])
    end
...
if Ret ~= 0
    disp('! Error at Analyze.SetPreference')
end
...
Weight = CreateVolumeMatrix(Element, WSection);
...
DataBase = repmat(empty_individual, MaxIt*nAnt, 1);

Q = NumOfAllElement;

Tau0 = 1/(mean([Element(:).Volume])*SteelWPerUnitV);
...
ViolationMin = WSection(1).Area/WSection(end).Area-1;

%% Initialization
disp('Initialization')

eta = 1./Weight;             % Heuristic Information Matrix

Tau = Tau0*ones(NumOfAllElement, NumOfWSection); % Phromone Matrix
Tau(1:Z,0.75*NumOfWSection:NumOfWSection)=10*Tau0;
Tau(Z+1:end,1:0.25*NumOfWSection)=10*Tau0;
...
for it = 1:MaxIt
    ...
    for k = 1:nAnt
        ...
        [ant(k).Cost, ant(k).Sol] = CostFunction(ant(k).Tour, Element, WSection);
    ...
    CheckNumber = 0;
    while (ant(1).Sol.Check == 0)
        CheckNumber = CheckNumber+1;
        disp(['It = ' num2str(it) ' ,CheckNumber = ' num2str(CheckNumber)])
        
        for l = 1:NumOfAllElement
            Ret = FrameObj.SetSection(Element(l).Name, WSection(ant(1).Tour(l)).Name);
            if Ret ~= 0
                disp(['! Error at 1024 It = ' num2str(MaxIt)]);
                disp(['! NumOfAllElement = ' num2str(l)]);  
            end 
        end
        
        Ret = Analyze.RunAnalysis();
        if Ret ~= 0
            disp(['! Error at 1032 It = ' num2str(MaxIt)]); 
        end
        ...
        DataBase(NFESAP).ElementSectionNumber = ant(1).Tour;
        DataBase(NFESAP).BeamSectionNumber = ant(1).Tour(1:Z);
        DataBase(NFESAP).ColumnSectionNumber = ant(1).Tour(Z+1:end);
    
        for i = 1:DesignedNumberItems
        ant(1).DesignRatio(i) = DesignedRatio(i);
        ...
        [ant(1).Cost, ant(1).Sol] = UpdateCostFunction(ant(1).Cost, ...
                            ant(1).DesignRatio, ViolationMin);
    ...
    % Update Phromones
    for k = 1:nAnt
        if ant(k).Sol.Check == 0
            ant(k).Cost = 0.5*ant(k).Cost*(1+ViolationMin);
        end
        Tour = ant(k).Tour;
    ...
    disp(['Iteration ' num2str(it) ': Best Cost = ' num2str(BestCost(it)) ': NFE = ' num2str(nfe(it)) ', NFESAP = ' num2str(nfesap(it))]);    
end
